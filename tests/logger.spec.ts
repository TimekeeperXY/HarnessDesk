...
describe('log redaction', () => {
  it('redacts common API key and authorization forms', () => {
    const value = redactSecrets('Authorization: Bearer abc123 api_key=xyz MIMO_API_KEY=mimo_secret_value sk-secretvalue')
    expect(value).not.toContain('abc123')
    expect(value).not.toContain('xyz')
    expect(value).not.toContain('mimo_secret_value')
